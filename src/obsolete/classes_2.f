!//////////////////////////////////////////////////////////////////////
!////  $Id: classes_2.f,v 1.1 2009/02/16 00:56:31 saroun Exp $
!////
...
! local variables useful during parsing
      ...
      character(LEN_ID),private :: ENUM_TYPE
...
C-------------------------------------------------------------
      integer function getEnumTypeOrd(ETYPE)
...
c          read(*,*)
        ...
        if (isCOMMAND) call startCOMMAND(tag,isize)
        return
99      error=.true.
...
!--------------------------------------------------------
      subroutine startCOMMAND(tag,isize)
! special handling of field start elements for COMMAND
!--------------------------------------------------------
      ...
      integer :: new_index,old_index
        ...
        case ('FLOAT','INT','STRING','NOYES','SIGN','ENUM')
        ! NARG and IDSTR count all argument types (incl. strings)
            myCMD%NARG=myCMD%NARG+1
            call APPENDPAR(myCMD%IDSTR,':',trim(PARAM_ID))
            old_index=myCMD%ARG_NUM(myCMD%NARG-1)
            new_index=old_index + isize
            myCMD%ARG_NUM(myCMD%NARG) = new_index
            myCMD%ARG(old_index+1:new_index)=0.D0
            select case(trim(tag))
            case('STRING')
              myCMD%NSARG=myCMD%NSARG+1
              call APPENDPAR(myCMD%SIDSTR,':',trim(PARAM_ID))
              call APPENDPAR(myCMD%SARG,':',' ')
            end select
        end select
